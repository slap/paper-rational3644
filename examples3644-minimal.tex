...
p1=0, is a line and a conic section q=0 (generally, the non-linear components of p1=0 is a (d-1)x(d-1) determinant) a[2,2]*a[3,3]-a[2,3]*a[3,2]=0
p2=0 and p3=0 are generally elliptic curves on the plane.
I wanted to know geometrically why generally the zero (over the field of algebraic numbers) Z(p1,p2,p3) consists of exactly 5 points. Experimentally I saw that the common points lie on the conic section p1=0. Generally Z(q,pj) j=2 or 3 are 6 points, but the extra point that is missing when we look at Z(q,p2,p3)=Z(p1,p2,p3) must have a meaning. Indeed: The intersection of the lines a[3,2]=0 and a[3,3]=0 is the "extra point" of Z(q,p2) and the intersection of the lines a[2,3]=0 and a[2,2]=0 is the "extra point" of Z(q,p3). Note that p3 is obtained by p1 by swapping a[3,2] with -a[2,2] and swapping a[3,3] with -a[2,3]. These are nice observations for which I think I can prove.

Recall that in many examples we had in 2019, Santiago (mostly) and I purposefully made the conic q=0 to be degenerate i.e. the union of two lines. In this way it is easier to construct (by choices of a[i,j]) Z(q,p2) or Z(q,p3) to be composed of 6 real points.
